...
async function tokenSendFlow(driver: Driver): Promise<void> {
  ...
  try {
    ...
    await driver.delay(1500); // Wait for navigation

    // Wait for send page to be ready - checkPageIsLoaded waits for inputRecipient (ens-input)
    ...
    try {
      ...
      if (isAssetPickerModalOpen) {
        // Select the first available token (ETH) from the asset picker
        // This matches the pattern in send-eth.spec.ts
        try {
          ...
          await driver.delay(500); // Wait for modal to close
        } catch (selectError) {
          // If token selection fails, try closing the modal
      ...
      // Modal check failed, continue anyway
    ...
    try {
      ...
      // Wait for next screen to load (confirmation page)
      await driver.waitForSelector(
        {
      ...
      // Cancel the transaction to avoid actually sending
      try {
        ...
        // Try back button if cancel not found
        try {
...
async function tokenSearchFlow(driver: Driver): Promise<void> {
  ...
  try {
    ...
    // Wait for send page to load - checkPageIsLoaded waits for inputRecipient (ens-input)
    // which is more reliably present than the asset picker button
    await sendTokenPage.checkPageIsLoaded();
    await driver.delay(1000);

    // Now wait for asset picker button to be present
    await driver.waitForSelector('[data-testid="asset-picker-button"]', {
      timeout: 10000,
    });

    // Click asset picker button to open the modal (matches swap-send-test-utils.ts pattern)
    const assetPicker = new AssetPicker(driver);
    await assetPicker.openAssetPicker('dest');
    await driver.delay(1000); // Wait for modal to open

    // Wait for search input in the asset picker modal
    const searchInputSelector =
      '[data-testid="asset-picker-modal-search-input"]';

    // Wait for search input to be located (no state requirement, just presence in DOM)
    await driver.waitForSelector(searchInputSelector, {
      timeout: 20000,
    });

    // Wait for token list to load via RPC
    await driver.delay(1000);

    // Click on the input to ensure it's focused and ready for input
    ...
    // Search for a token - type character by character to simulate real typing
    // Get fresh element reference each time (as done in swap-send-test-utils)
    for (const char of 'ETH') {
      const input = await driver.waitForSelector(searchInputSelector, {
        state: 'enabled',
      });
      await input.sendKeys(char);
      await driver.delay(100); // Small delay between characters
    }
    await driver.delay(1500); // Wait for search results to appear
    ...
    try {
      ...
      // If clear button not found, clear using keyboard shortcut
      const input = await driver.waitForSelector(searchInputSelector, {
        state: 'enabled',
      });
    ...
    // Click on input again to ensure focus for second search
    ...
    // Search for another token - type 'USDC' character by character
